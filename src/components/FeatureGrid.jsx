export const FeatureGrid = () => {
    ...
    return (
      <section className="bg-white py-16 px-6 md:px-12 mt-10 text-center">
        <h2 className="text-3xl md:text-4xl font-bold text-[var(--text-color)] mb-3 leading-[56px] ">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-30 gap-x-8 max-w-6xl mx-auto">
          {features.map((feature, index) => (
            <div key={index} className=" mt-4 flex flex-col items-center text-center px-4">
                <img src="/images/gridLogo.png" alt=""  className="mx-auto my-7"/>
              
              <h3 className="font-semibold text-xl text-[var(--text-color)] mb-4 leading-[32px]">{feature.title}</h3>
              <p className="text-[var(--content-color)] text-sm">{feature.desc}</p>
            </div>
          ))}
        </div>
      </section>
